...
load("data_output/models_subset.RData")
d2_subset = readRDS("data_output/d_subset_for_stat.rds")
d2_ave_std_subset = readRDS("data_output/d_subset_ave_std.rds")
names(d2)

group_by(d2_subset, taxon) |> summarise(nsp = n_distinct(sp))
group_by(d2, taxon) |> summarise(nsp = n_distinct(sp))
summary(m_onset_final_subset_same_as_allsp)


m_final_coef = 
  bind_rows(broom.mixed::tidy(m_onset_final_subset_same_as_allsp) |> mutate(Phenology = "Onset"),
            broom.mixed::tidy(m_offset_final_subset_same_as_allsp) |> mutate(Phenology = "Offset"),
            broom.mixed::tidy(m_duration_final_subset_same_as_allsp) |> mutate(Phenology = "Duration")
  ) |> 
  filter(effect == "fixed") |> 
...
r2s = bind_rows(
  MuMIn::r.squaredGLMM(m_onset_final_subset_same_as_allsp) |> as.data.frame() |> mutate(Phenology = "Onset"),
  MuMIn::r.squaredGLMM(m_offset_final_subset_same_as_allsp) |> as.data.frame() |> mutate(Phenology = "Offset"),
  MuMIn::r.squaredGLMM(m_duration_final_subset_same_as_allsp) |> as.data.frame() |> mutate(Phenology = "Duration")
) |> 
...
summary(m_onset_final_subset_same_as_allsp)
summary(m_onset_final_2_subset_same_as_allsp)

summary(m_offset_final_subset_same_as_allsp)
summary(m_offset_final_2_subset_same_as_allsp)

summary(m_duration_final_subset_same_as_allsp)
summary(m_duration_final_2_subset_same_as_allsp)
...
d1_onset = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_warm_days_before_onset_logit",
                                         ...
                                         mod = m_onset_final_subset_same_as_allsp, se = T, 
  ...
  mutate(`Extreme warm` = `Extreme warm` * filter(d2_ave_std_subset, var == "prop_extreme_warm_days_before_onset_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_warm_days_before_onset_logit")$ave,
...
onset_p1 = ggplot(d1_onset, aes(x = `Extreme warm`, y = fit, group = `Extreme dry`,
  ...
  labs(y = "", x = "Number of extreme warm days", tag = "E") +
...
d2_onset = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_wet_days_before_onset_logit", 
  ...
  mutate(Precip. = recode(Precip., "-1.5" = "-1.5 sd", "0" = "0 (mean)", "1.5" = "1.5 sd"),
         ...
         taxon = rtrees:::cap_first_letter(taxon),
         `Sig. Inter.` = ifelse(taxon == "Plant", "No", "Yes")) |> 
  mutate(`Extreme wet` = `Extreme wet` * filter(d2_ave_std_subset, var == "prop_extreme_wet_days_before_onset_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_wet_days_before_onset_logit")$ave,
...
onset_p2 = ggplot(d2_onset, aes(x = `Extreme wet`, y = fit, group = Precip.,
  ...
  scale_fill_manual(values = color_values) +
  scale_color_manual(values = color_values) +
  labs(y = "", x = "Number of extreme wet days", tag = "C") +
  cowplot::theme_cowplot() +
  theme(legend.position = c(0.51, 0.85), legend.box = "horizontal") 
...
d3_onset = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_dry_days_before_onset_logit", 
  ...
  mutate(Precip. = recode(Precip., "-1.5" = "-1.5 sd", "0" = "0 (mean)", "1.5" = "1.5 sd"),
         ...
         taxon = rtrees:::cap_first_letter(taxon),
         `Sig. Inter.` = ifelse(taxon == "Plant", "No", "Yes")) |> 
  mutate(`Extreme dry` = `Extreme dry` * filter(d2_ave_std_subset, var == "prop_extreme_dry_days_before_onset_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_dry_days_before_onset_logit")$ave,
...
onset_p3 = ggplot(d3_onset, aes(x = `Extreme dry`, y = fit, group = Precip.,
  ...
  labs(y = "Onset (day of year)", x = "Number of extreme dry days", tag = "D (NS)") +
  cowplot::theme_cowplot() +
  theme(legend.position = c(0.51, 0.8), legend.box = "horizontal") 
...
d4_onset = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_cold_days_before_onset_logit", 
  ...
  mutate(Temp. = recode(Temp., "-1.5" = "-1.5 sd", "0" = "0 (mean)", "1.5" = "1.5 sd"),
         ...
         taxon = rtrees:::cap_first_letter(taxon),
         `Sig. Inter.` = ifelse(taxon == "Plant", "No", "Yes")) |> 
  mutate(`Extreme cold` = `Extreme cold` * filter(d2_ave_std_subset, var == "prop_extreme_cold_days_before_onset_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_cold_days_before_onset_logit")$ave,
...
onset_p4 = ggplot(d4_onset, aes(x = `Extreme cold`, y = fit, group = Temp.,
  ...
  scale_fill_manual(values = color_values) +
  scale_color_manual(values = color_values) +
  labs(y = "Onset (day of year)", x = "Number of extreme cold days", tag = "B (NS)") +
  cowplot::theme_cowplot() +
  theme(legend.position = c(0.51, 0.87), legend.box = "horizontal") 

# no diff between insect and plant, not plotting it?
plot_model(m_onset_final_subset_same_as_allsp, type = "pred", terms = c("prop_extreme_warm_days_before_onset_logit", "tmean_5_yr_ave"), show.data = T)
...
x_insect = confint(m_onset_final_subset_same_as_allsp, level = 0.95, method = "Wald", parm = val)
x_plant = confint(m_onset_final_2_subset_same_as_allsp, level = 0.95, method = "Wald", parm = val)

x_insect_2 = broom::tidy(m_onset_final_subset_same_as_allsp) |> 
  filter(effect == "fixed", term %in% val) |> 
  dplyr::select(-effect, -group) |> 
  left_join(rownames_to_column(as.data.frame(x_insect), "term"), by = "term") |> 
  left_join(rownames_to_column(as.data.frame(confint(m_onset_final_subset_same_as_allsp, level = 0.85, method = "Wald", parm = val)), "term"), by = "term") |> 
  mutate(taxon = "Insect")

x_plant_2 = broom::tidy(m_onset_final_2_subset_same_as_allsp) |> 
  filter(effect == "fixed", term %in% val) |> 
  dplyr::select(-effect, -group) |> 
  left_join(rownames_to_column(as.data.frame(x_plant), "term"), by = "term") |> 
  left_join(rownames_to_column(as.data.frame(confint(m_onset_final_2_subset_same_as_allsp, level = 0.85, method = "Wald", parm = val)), "term"), by = "term") |> 
  mutate(taxon = "Plant")
...
p_coef_final_onset = ggplot(x_both_onset, aes(x = estimate, y = term, color = taxon)) +
  ...
  geom_linerange(position = position_dodge(width = 0.3), 
                 ...
                 linewidth = 2, 
                 alpha = ifelse(x_both_onset$p.value < 0.05, 1, 0.2)) +
  labs(x = "Onset Coefficients", y = "", color = "Taxon", tag = "A") +
...
onset_pall = p_coef_final_onset + onset_p4 + onset_p2 + onset_p3 + onset_p1 +
  plot_layout(design = layout) #+ plot_annotation(tag_levels = "A", tag_suffix = c("", "*", "", "*", ""))

# onset_pall = cowplot::plot_grid(onset_p3, onset_p2, onset_p1, onset_p4, nrow = 2, labels = "AUTO")
ggsave("figures/fig_3_onset_prop_subset.pdf", plot = onset_pall, width = 15, height = 9.5)
ggsave("figures/fig_3_onset_prop_subset.png", plot = onset_pall, width = 15, height = 9.5)

# fig offset ----
d1_offset = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_cold_days_within_duration_logit", 
                                                               "tmean_5_yr_ave", "taxon"), 
                                          mod = m_offset_final_subset_same_as_allsp, se = T, 
  ...
  mutate(`Extreme cold` = `Extreme cold` * filter(d2_ave_std_subset, var == "prop_extreme_cold_days_within_duration_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_cold_days_within_duration_logit")$ave,
...
d2_offset = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_wet_days_within_duration_logit", "precip_5_yr_ave"), 
  ...
  mutate(Precip. = recode(Precip., "-1.5" = "-1.5 sd", "0" = "0 (mean)", "1.5" = "1.5 sd"),
         Precip. = as.factor(Precip.)) |> 
  mutate(`Extreme wet` = `Extreme wet` * filter(d2_ave_std_subset, var == "prop_extreme_wet_days_within_duration_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_wet_days_within_duration_logit")$ave,
...
d3_offset = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_warm_days_within_duration_logit", 
  ...
  mutate(`Extreme dry` = recode(`Extreme dry`, "-1.5" = "-1.5 sd", "0" = "0 (mean)", "1.5" = "1.5 sd"),
         ...
         taxon = rtrees:::cap_first_letter(taxon),
         `Sig. Inter.` = ifelse(taxon == "Plant", "No", "No")) |> 
  mutate(`Extreme warm` = `Extreme warm` * filter(d2_ave_std_subset, var == "prop_extreme_warm_days_within_duration_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_warm_days_within_duration_logit")$ave,
...
offset_p4 = ggplot(d4_offset, aes(x = `Extreme warm`, y = fit, group = `Extreme wet`,
                                  fill = `Extreme wet`)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = ribbon_alpha) +
  geom_line(aes(color = `Extreme wet`), linewidth = line_linewidth, linetype = 1) +
  scale_fill_manual(values = color_values) +
  scale_color_manual(values = color_values) +
...
x_insect = confint(m_offset_final_subset_same_as_allsp, level = 0.95, method = "Wald", parm = val)
x_plant = confint(m_offset_final_2_subset_same_as_allsp, level = 0.95, method = "Wald", parm = val)

x_insect_2 = broom::tidy(m_offset_final_subset_same_as_allsp) |> 
  filter(effect == "fixed", term %in% val) |> 
  dplyr::select(-effect, -group) |> 
  left_join(rownames_to_column(as.data.frame(x_insect), "term"), by = "term") |> 
  left_join(rownames_to_column(as.data.frame(confint(m_offset_final_subset_same_as_allsp, level = 0.85, method = "Wald", parm = val)), "term"), by = "term") |> 
  mutate(taxon = "Insect")

x_plant_2 = broom::tidy(m_offset_final_2_subset_same_as_allsp) |> 
  filter(effect == "fixed", term %in% val) |> 
  dplyr::select(-effect, -group) |> 
  left_join(rownames_to_column(as.data.frame(x_plant), "term"), by = "term") |> 
  left_join(rownames_to_column(as.data.frame(confint(m_offset_final_2_subset_same_as_allsp, level = 0.85, method = "Wald", parm = val)), "term"), by = "term") |> 
  mutate(taxon = "Plant")
...
ggsave("figures/fig_4_offset_prop_subset.pdf", plot = offset_pall, width = 15, height = 9.5)
ggsave("figures/fig_4_offset_prop_subset.png", plot = offset_pall, width = 15, height = 9.5)

# duration fig ----
d1_duration = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_wet_days_total_logit", "precip_5_yr_ave", "taxon"), 
                                            mod = m_duration_final_subset_same_as_allsp, se = T, 
  ...
  mutate(`Extreme wet` = `Extreme wet` * filter(d2_ave_std_subset, var == "prop_extreme_wet_days_total_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_wet_days_total_logit")$ave,
         `Extreme wet` = logit_back_to_prop(`Extreme wet`),
...
duration_p1 = ggplot(d1_duration, aes(x = `Extreme wet`, y = fit, group = `Precip.`,
  ...
  labs(y = "Duration (number of days)",
       x = "Proportion of extreme wet days", tag = "D") +
  cowplot::theme_cowplot() +
...
d2_duration = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_dry_days_total_logit", "precip_5_yr_ave", "taxon"), 
  ...
  mutate(`Precip.` = recode(`Precip.`, "-1.5" = "-1.5 sd", "0" = "0 (mean)", "1.5" = "1.5 sd"),
         ...
         taxon = rtrees:::cap_first_letter(taxon),
         `Sig. Inter.` = ifelse(taxon == "Plant", "No", "No")) |> 
  mutate(`Extreme dry` = `Extreme dry` * filter(d2_ave_std_subset, var == "prop_extreme_dry_days_total_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_dry_days_total_logit")$ave,
...
duration_p2 = ggplot(d2_duration, aes(x = `Extreme dry`, y = fit, group = `Precip.`,
  ...
  # scale_color_viridis_d() +
  labs(y = "",
       x = "Proportion of extreme dry days", tag = "E") +
  cowplot::theme_cowplot() +
  theme(legend.position = c(0.52, 0.8), legend.box = "horizontal") 
...
d3_duration = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_warm_days_total_logit", 
  ...
  mutate(`Extreme wet` = recode(`Extreme wet`, "-1.5" = "-1.5 sd", "0" = "0 (mean)", "1.5" = "1.5 sd"),
         ...
         taxon = rtrees:::cap_first_letter(taxon),
         `Sig. Inter.` = ifelse(taxon == "Plant", "Yes", "No")) |> 
  mutate(`Extreme warm` = `Extreme warm` * filter(d2_ave_std_subset, var == "prop_extreme_warm_days_total_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_warm_days_total_logit")$ave,
         `Extreme warm` = logit_back_to_prop(`Extreme warm`),
         fit = exp(fit), lower = exp(lower), upper = exp(upper))
...
duration_p4 = ggplot(d4_duration, aes(x = `Extreme warm`, y = fit, group = `Temp.`,
  ...
  # scale_color_viridis_d() +
  labs(y = "Duration (number of days)",
       x = "Proportion of extreme warm days", tag = "B") +
  cowplot::theme_cowplot() +
...
d5_duration = as.data.frame(effects::Effect(focal.predictors = c("prop_extreme_cold_days_total_logit", 
  ...
  mutate(`Extreme cold` = `Extreme cold` * filter(d2_ave_std_subset, var == "prop_extreme_cold_days_total_logit")$std +
           filter(d2_ave_std_subset, var == "prop_extreme_cold_days_total_logit")$ave,
         `Extreme cold` = logit_back_to_prop(`Extreme cold`),
         fit = exp(fit), lower = exp(lower), upper = exp(upper))
duration_p5 = ggplot(d5_duration, aes(x = `Extreme cold`, y = fit, group = `Temp.`,
  ...
  # scale_fill_viridis_d() +
  # scale_color_viridis_d() +
  labs(y = "", x = "Proportion of extreme cold days", tag = "C (NS)") +
  cowplot::theme_cowplot() +
  theme(legend.position = c(0.52, 0.8), legend.box = "horizontal") 
...
x_insect = confint(m_duration_final_subset_same_as_allsp, level = 0.95, method = "Wald", parm = val)
x_plant = confint(m_duration_final_2_subset_same_as_allsp, level = 0.95, method = "Wald", parm = val)

x_insect_2 = broom::tidy(m_duration_final_subset_same_as_allsp) |> 
  filter(effect == "fixed", term %in% val) |> 
  dplyr::select(-effect, -group) |> 
  left_join(rownames_to_column(as.data.frame(x_insect), "term"), by = "term") |> 
  left_join(rownames_to_column(as.data.frame(confint(m_duration_final_subset_same_as_allsp, level = 0.85, method = "Wald", parm = val)), "term"), by = "term") |> 
  mutate(taxon = "Insect")

x_plant_2 = broom::tidy(m_duration_final_2_subset_same_as_allsp) |> 
  filter(effect == "fixed", term %in% val) |> 
  dplyr::select(-effect, -group) |> 
  left_join(rownames_to_column(as.data.frame(x_plant), "term"), by = "term") |> 
  left_join(rownames_to_column(as.data.frame(confint(m_duration_final_2_subset_same_as_allsp, level = 0.85, method = "Wald", parm = val)), "term"), by = "term") |> 
  mutate(taxon = "Plant")
...
p_coef_final_duration = ggplot(x_both_duration, aes(x = estimate, y = term, color = taxon)) +
  ...
  geom_linerange(position = position_dodge(width = 0.3), 
                 ...
                 linewidth = 2, 
                 alpha = ifelse(x_both_duration$p.value < 0.05, 1, 0.2)) +
  labs(x = "Duration Coefficients", y = "", color = "Taxon", tag = "A") +
  # scale_x_continuous(breaks = c(-15, -10, -5, 0, 5)) +
  cowplot::theme_cowplot() +
  theme(legend.position = "top") 
...
duration_pall = p_coef_final_duration + duration_p4 + duration_p5 + duration_p1 + duration_p2 +
  plot_layout(design = layout)# + plot_annotation(tag_levels = 'A')
ggsave("figures/fig_5_duration_prop_subset.pdf", plot = duration_pall, width = 15, height = 9.5)
ggsave("figures/fig_5_duration_prop_subset.png", plot = duration_pall, width = 15, height = 9.5)
